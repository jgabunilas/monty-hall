...
# Import dependencies - matplotlib, the random module, and panda
...
while True:
    playstyle = input(
        "Would you like to (1) play manually, or (2) run a simulation? (choose 1 or 2) "
    )
...
if int(playstyle) == 1:
    ...
    while True:
        choice = input(
            "Before you are three doors. Behind one of them is a new car. Behind the other two is a goat. You will win whatever is behind the door you choose! Please choose a door (1, 2, or 3): "
        )
    ...
    print(
        f"Before we open Door #{choice}, let's open one of the doors that you did not choose!"
    )
    if int(choice) == 1:
        ...
        if choices[other_doors[0]] == "goat":
            print(
                f"We have opened Door #{other_doors[0] + 1}, which reveals a goat! Good job dodging that one!"
            )
            remaining_door = other_doors[1]
        else:
            print(
                f"We have opened Door #{other_doors[1] + 1}, which reveals a goat! Good job dodging that one!"
            )
    ...
    while True:
        switch = input(
            f"Would you like to switch to Door #{remaining_door + 1} (y/n)?: "
        )
        if switch == "y":
            prize = choices[remaining_door]
            print(
                f"You have chosen to switch to Door #{remaining_door + 1}. The door opens and... You have won a {prize}!"
            )
            print(
                f"Your original selection of Door #{choice} was a {choices[int(choice) - 1]}."
            )
            break
        elif switch == "n":
            print(
                f"You have chosen to stay with Door #{choice}. The door opens and... You have won a {choices[int(choice) - 1]}!"
            )
            break
        else:
...
## Simulation
    ...
    while True:
        num_sims = input(
            "Please enter the number of simulations you would like to run: "
        )
        try:
    ...
    while True:
        switch = input(
            "Please indicate whether the simulator will switch to the remaining door after one of the unchosen doors is opened (y/n) "
        )
    ...
    df = pd.DataFrame(
        data={
            "Total Prizes": [total_car_prizes, total_goat_prizes],
            "Percent": [str(percent_cars) + "%", str(percent_goats) + "%"],
        },
        index=["Cars", "Goats"],
    )
    # print(df)
    ...
    ax = df.plot(
        kind="bar", title=f"Prize Results for {num_sims} Simulations, switch = {switch}"
    )
    plt.show()
